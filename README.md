...
LoadModel函数用于解析含有MODEL关键词的PDB，其参数与Load函数一致。函数返回含有多
个蛋白对象的list，这些蛋白对象的name属性将被设置为：PDBID_model_MODELID：

structObjList = PDBTools.LoadModel('xxx.pdb', True)  # 解析含有多个MODEL的PDB

请注意，不含MODEL关键词的PDB文件不可使用LoadModel函数进行解析。
...
非Atom层级对象可调用DumpFasta方法，将层级对象输出为Fasta文件。同时，由多个层级对
象构成的list可调用DumpFastal函数，将list中所有的层级对象依次输出到一个Fasta文件。
此外，非Atom层级对象可访问fasta属性，得到字符串形式的Fasta文件内容。需要注意的是，
所有层级fasta格式的第一行都是取层级对象所对应的name属性的内容，而第二行为seq属性
的内容。
DumpFasta、DumpFastal的参数与Dump、Dumpl完全一致。
...
此属性进行访问与修改。以及ins属性，用于存储转变位置指示符。
...
resObj.compNum = ('1', '')  # 将当前残基完整编号修改为1（无插入字符）

非Atom层级可以调用RenumResidues与RenumAtoms方法，分别对残基和原子进行重编号。该
方法接受一个默认值为1的可选参数，用于指定重编号的起始编号：

structObj.RenumResidues()    # 对残基进行重编号，从1开始编号
structObj[0].RenumAtoms(10)  # 对原子进行重编号，从10开始编号
...
* 空字段
由于解析时会对所有字符串字段（包括链名、残基名、残基插入字符、原子名、转变位置指
示符）进行strip方法处理，故所有的空字段，如空链名，空插入字符等，都会被解析为空
字符串。此性质在筛选及属性修改时可能有用。
...
* MODEL关键词
对于Load函数，其解析时不会关注含有多个MODEL的PDB。除ATOM以外的任何关键词都将被忽略。
